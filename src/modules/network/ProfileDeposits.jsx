...
export const ProfileDeposits = ({ userData, currentSkin }) => {
  ...
  // 🛡️ PILARES DE SEGURIDAD (Matriz de Permisos)
  const canReadGrid = hasPermission(currentSkin.id, 'net_deposit_read'); // Fila 3
  const canWriteDeposit = hasPermission(currentSkin.id, 'net_deposit_write'); // Fila 4

  // Liquidez de la Bóveda Central (Skin)
  const vaultBalance = skinBalances?.[currentSkin.id] || 0;

  // Estados del Formulario de Fondeo
  ...
  // Extracción pura de transacciones solo para ESTE usuario, ESTA Skin y de TIPO Depósito
  ...
  const handleDepositSubmit = (e) => {
    ...
    if (!canWriteDeposit) return;

    const numAmount = parseFloat(amount);
    if (walletType === 'REAL' && numAmount > vaultBalance) {
      setErrorMsg(`Fondos insuficientes en la Bóveda de ${currentSkin.name}.`);
      return;
    }
    ...
    setTimeout(() => {
      ...
      setAuditNote('');
      alert(
        `✅ Fondeo exitoso. Se inyectaron ${formatMoney(
          numAmount,
  ...
  const filteredDeposits = recentDeposits.filter((dep) =>
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        <div className="xl:w-[400px] flex-shrink-0 flex flex-col gap-4">
          <div className="flex items-center gap-3 mb-1">
            <ArrowDownToLine className="text-emerald-500" size={18} />
            ...
            {!canWriteDeposit && (
              <div className="absolute inset-0 z-10 bg-[#111827]/90 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center border border-slate-800/50 rounded-xl">
                ...
                <p className="text-[11px] text-slate-400 max-w-[250px]">
                  ...
                  <span className="text-[#D10057] font-mono">
                    net_deposit_write
                  </span>
                  .
                </p>
            ...
            <form onSubmit={handleDepositSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    step="0.01"
                    required
                    disabled={!canWriteDeposit || isSubmitting}
                    value={amount}
                    onChange={(e) => setAmount(e.target.value)}
                    className={`w-full bg-[#1F2937] border ${
                      errorMsg ? 'border-red-500' : 'border-slate-700'
                    } text-white text-base py-2 pl-12 pr-3 rounded-lg outline-none focus:border-emerald-500 transition-colors font-mono font-bold disabled:opacity-50`}
                    placeholder="0.00"
                  />
                </div>
                {errorMsg && (
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!canWriteDeposit || isSubmitting}
                  className="w-full bg-[#1F2937] border border-slate-700 text-slate-200 text-sm py-2 px-3 rounded-lg outline-none focus:border-emerald-500 transition-colors disabled:opacity-50"
                >
                  <option value="REAL">Saldo Real (Debita de la Bóveda)</option>
                  <option value="BONO">Saldo Promocional (Bono)</option>
                </select>
              </div>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider flex items-center justify-between">
                  <span className="flex items-center gap-1">
                    Nota de Auditoría{' '}
                    <Info size={12} className="text-[#D10057]" />
                  </span>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Justifique el fondeo manual..."
                  className="w-full bg-[#1F2937] border border-slate-700 text-white text-sm py-2 px-3 rounded-lg outline-none focus:border-emerald-500 transition-colors resize-none disabled:opacity-50 custom-scrollbar"
                />
              </div>
              ...
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-start gap-3">
                ...
                <p className="text-[10px] text-amber-400/90 leading-relaxed">
                  ...
                  Autorizador de este movimiento.
                </p>
              </div>
              ...
              <button
                type="submit"
                disabled={
                  !canWriteDeposit || isSubmitting || !amount || !auditNote
                }
                className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold text-sm transition-colors shadow-lg shadow-emerald-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  'Procesando...'
                ) : (
                  <>
                    <CheckCircle2 size={16} /> Confirmar Depósito
                  </>
                )}
              </button>
            </form>
            ...
            {!canReadGrid ? (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-xs text-slate-500 max-w-xs">
                  No tienes permiso para auditar el historial de depósitos.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-left text-xs whitespace-nowrap">
                  <thead className="bg-[#0f1522] text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-800 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right">Monto Inyectado</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    {filteredDeposits.length > 0 ? (
                      ...
                      <tr>
                        ...
                        >
                          No se encontraron registros de fondeo administrativo.
                        </td>
                      </tr>
                    )}
                  </tbody>
